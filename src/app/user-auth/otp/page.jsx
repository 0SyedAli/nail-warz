...
export default function UserVerifyOtpPage() {
    ...
    useEffect(() => {
        ...
        if (storedToken) {
            ...
            router.push("/user-auth/login"); // no context → go back
        }
    }, [router]);
    ...
    const verifyOTP = async () => {
        ...
        try {
            ...
            if (flow === "signup") {
                endpoint = "/VerifyOtpAndCreate"; // Updated user signup OTP endpoint
                payload = { token, otp: Number(code) }; // Number cast as per prompt
            } else if (flow === "forgot") {
                endpoint = "/verifyOtp"; // Keeping admin one for forgot unless changed
